...
const opHome=function(a){MgrUi.showSommario();toggleMenu();UaWindowAdm.closeAll()},opArchivio=function(a){MgrUi.showIndici();toggleMenu();UaWindowAdm.closeAll()},opHelp=function(){toggleMenu();UaWindowAdm.closeAll();wnds.wdiv.open("./html/help0.html")},opNews=function(a){toggleMenu();UaWindowAdm.closeAll();wnds.wdiv.open("./html/help1.html")},opRedazione=function(a){alert("readzione")},opCollaboatori=function(a){alert("collaboratori")},opIPOTESI=function(a){alert("IPOTESI")},opIPubblicita=function(a){alert("pubblicita")},
op9=async function(a){alert("op0")},op10=async function(a){alert("op10")};function initMenu(){document.body.classList.add("theme-light");const a=document.querySelector(".menu-h");a.addEventListener("click",toggleMenu);document.addEventListener("keydown",c=>{"Escape"===c.key&&a.classList.contains("active")&&toggleMenu()})}function toggleMenu(){document.querySelector(".menu-h").classList.toggle("active");document.body.classList.toggle("open-menu")}
function getTheme(){const a=localStorage.getItem("theme");a&&"dark"==a&&setDark()}function setLight(){document.documentElement.classList.toggle("invert");document.body.classList.remove("theme-dark");document.body.classList.add("theme-light");localStorage.setItem("theme","ligth")}function setDark(){document.documentElement.classList.toggle("invert");document.body.classList.remove("theme-light");document.body.classList.add("theme-dark");localStorage.setItem("theme","dark")}
function invertColors(){document.querySelectorAll("*").forEach(a=>{a.classList.add("invert-colors")})}function enableEsc(a){document.addEventListener("keydown",cmdEsc);a&&("Escape"!==a.key&&27!==a.keyCode||cmdEsc())}function disableEsc(a){document.removeEventListener("keydown",cmdEsc)}function cmdEsc(){closeReader()}
const imageCarousel=()=>{const a=document.querySelector(".col-right");a.querySelectorAll(".item0").forEach(c=>{c.classList.add("active")});setInterval(()=>{const c=a.querySelector(".item0");a.appendChild(c)},5E3)};let fontSize=18;var textCurrent="";const fh=a=>`
...
`,showReader=a=>{textCurrent=a;h=fh(a);a=UaWindowAdm.create("id_reader");a.setZ(12);a.vw_vh().setXY(.1,10,-1);a.setHtml(h);a.show()};function openReader(a){fetchText(a,showReader);enableEsc()}
...
function increaseFontSize(){const a=document.getElementById("content");fontSize=Math.min(fontSize+2,32);a.style.fontSize=`${fontSize}px`}function decreaseFontSize(){const a=document.getElementById("content");fontSize=Math.max(fontSize-2,12);a.style.fontSize=`${fontSize}px`}function openFullscreen(){document.fullscreenElement?document.exitFullscreen&&document.exitFullscreen():document.documentElement.requestFullscreen()};function toggleReading(){tts.toggleReading()}function stopReading(){tts.stopReading()}function closeSpeak(){tts.closeSpeak()}function toggleSpeak(){tts.toggleSpeak()}function saveSpeak(){tts.saveSpeak()}
...
<div class="div-text">${c}</div>
    ...
    `;this.w.drag();this.w.setZ(12);this.w.vw_vh().setXY(0,10,-1);this.w.setHtml(c);this.w.show()},close(){this.w.close()},open(c){fetchText(c,b=>this.show(b))}}),wnds={wdiv:null,init(){this.wdiv=WndDiv("id_w0")},closeAll(){UaWindowAdm.close("id_w0")}},sortSchede=a=>{a.schede.sort((c,b)=>{c=parseInt(c.id,10);b=parseInt(b.id,10);return c-b});return a},MgrUi={ipotesi:[],htmlIndici:null,htmlSommario:null,numeri:[],async init(){const a=await getJson("./data/ipotesi.json");this.setting(a.numeri)},async setting(a){this.ipotesi=
[];this.numeri=a;for(const c of this.numeri)a=await getJson(`./data/${c}/sommario.json`),a=sortSchede(a),this.ipotesi.push(a);this.buildHtml();this.showSommario()},buildHtml(){const a=(f,k)=>{f=`./data/${f}/${k.file}`;return k.img?`
        <div class="list-item">
