...
()=>this.saveSpeak());EventManager$$module$javascript$event_manager.on("click","#btn-close-speak",()=>this.closeSpeak());EventManager$$module$javascript$event_manager.on("input",'#id_voices input[type="range"]',(a,b)=>{if(a=b.parentElement.querySelector("span"))a.textContent=b.value})}populateVoiceList(){this.voices=this.synth.getVoices().filter(a=>a.lang.startsWith("it"))}openSpeak(){if(this.synth){this.panel=!0;var a=this.loadSettings(),b=`
      ...
      `,c=UaWindowAdm$$module$javascript$uawindow.create("id_speak");c.setZ(1E3);c.setHtml(b);c.vw_vh().setXY(30,50,-1);c.show();c.drag();this.setSpeak()}else alert("La sintesi vocale non \u00e8 supportata da questo browser.")}setSpeak(){const a=this.loadSettings(),b=document.getElementById("id_voices");b&&(b.querySelector('[name="amplitude"]').value=a.amplitude,b.querySelector('[name="pitch"]').value=a.pitch,b.querySelector('[name="speed"]').value=a.speed,b.querySelector('[name="amplitude"]').parentElement.querySelector("span").innerText=
a.amplitude,b.querySelector('[name="pitch"]').parentElement.querySelector("span").innerText=a.pitch,b.querySelector('[name="speed"]').parentElement.querySelector("span").innerText=a.speed)}closeSpeak(){this.saveSpeak();UaWindowAdm$$module$javascript$uawindow.close("id_speak");this.panel=!1}toggleSpeak(){this.panel?this.closeSpeak():this.openSpeak()}saveSpeak(){var a=document.getElementById("id_voices");if(a){var b=a.querySelector('[name="voice"]').value,c=a.querySelector('[name="amplitude"]').value,
d=a.querySelector('[name="pitch"]').value;a=a.querySelector('[name="speed"]').value;this.saveSettings(b,c,d,a)}}saveSettings(a,b,c,d){localStorage.setItem("voice",a);localStorage.setItem("amplitude",b);localStorage.setItem("pitch",c);localStorage.setItem("speed",d)}loadSettings(){const a={};a.voice=localStorage.getItem("voice")||"default";a.amplitude=localStorage.getItem("amplitude")||1;a.pitch=localStorage.getItem("pitch")||1;a.speed=localStorage.getItem("speed")||1;for(let b in a)"undefined"===
a[b]&&(a[b]="1");return a}stopReading(){if(this.isReading){const a=document.getElementById("btn-toggle-reading");a&&(a.innerText="\u25b6");this.synth.cancel();this.isReading=!1}}toggleReading(a){if(this.synth)if(this.textCurrent=a,this.isReading)this.stopReading();else{const b=document.getElementById("btn-toggle-reading");b&&(b.innerText="\u23f8");this.isReading=!0;const c=this.loadSettings();a=this.cleanupText(this.textCurrent);a=new SpeechSynthesisUtterance(a);const d=this.voices.find(f=>f.name===
c.voice);d&&(a.voice=d);a.volume=c.amplitude;a.rate=c.speed;a.pitch=c.pitch;a.lang="it-IT";const e=()=>{this.isReading=!1;b&&(b.innerText="\u25b6")};a.onend=e;a.onerror=f=>{console.error("Errore nella sintesi vocale:",f.error);e()};this.synth.speak(a)}}cleanupText(a){return a.replace(/<[^>]+>/g,"").replace(/&[^;]+;/g,"").replace(/[#@*]/g,"")}}const tts$$module$javascript$ipotesi_speak=new TextToSpeech$$module$javascript$ipotesi_speak;var module$javascript$ipotesi_speak={tts:tts$$module$javascript$ipotesi_speak};class Reader$$module$javascript$ipotesi_reader{constructor(){this.fsizeDef=this.fontSize=18;this.fsizeMax=32;this.fsizeMin=14;this.fsizeKey="fsize_reader";this.urlCurrent=this.textCurrent="";this.isReaderOpen=!1;this.registerEventHandlers()}registerEventHandlers(){EventManager$$module$javascript$event_manager.on("click","#btn-read-pdf",()=>this.readPDF());EventManager$$module$javascript$event_manager.on("click","#btn-toggle-speak",()=>tts$$module$javascript$ipotesi_speak.toggleSpeak());EventManager$$module$javascript$event_manager.on("click",
