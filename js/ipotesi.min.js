...
`,showReader=a=>{textCurrent=a;h=fh(a);a=UaWindowAdm.create("id_reader");a.setZ(12);a.vw_vh().setXY(.1,10,-1);a.setHtml(h);a.show();defaultFontSize()};function openReader(a){console.log(a);urlCurrent=a;fetchText(a,showReader);enableEsc()}
...
[];this.numeri=a;for(const c of this.numeri.reverse())a=await getJson(`./data/${c}/sommario.json`),a=sortSchede(a),this.ipotesi.push(a);this.buildHtml();this.showSommario()},buildHtml(){const a=(e,k)=>{e=`./data/${e}/${k.file}`;return k.img?`
        <div class="list-item">
        <a href="#" onclick="openReader('${e}')"><strong>${k.titolo}</strong></a>
        <p>${k.sottotitolo}</p>
        ...
        <p>${k.sottotitolo}</p>
        <p>${k.autore}</p>
        </div>`},c=UaJtfh();c.init();c.append('<div class="list">');for(var b=0;b<this.ipotesi.length;b++){var d=this.ipotesi[b],f=this.numeri[b];c.append(`
      <div class="num">Numero: ${f.replace(/^0+/,"")}</div>
      `);d=d.schede;for(const e of d)c.append(a(f,e))}c.append("</div>");this.htmlIndici=c.html();b=this.numeri[0];f=this.ipotesi[0];c.init();c.append('<div class="list">');f=f.schede;for(const e of f)c.append(a(b,e));c.append("</div>");this.htmlSommario=c.html()},showSommario(){document.getElementById("id_item1").innerHTML=MgrUi.htmlSommario},showIndici(){document.getElementById("id_item1").innerHTML=MgrUi.htmlIndici}};const nodrag_tds=["input","select","a"],nodrag_cls="nodrag";
var UaDrag=function(a){return function(c){let b=0,d=0,f=0,e=0;const k=function(g){g=g||window.event;g.preventDefault();b=f-g.clientX;d=e-g.clientY;f=g.clientX;e=g.clientY;c.style.top=c.offsetTop-d+"px";c.style.left=c.offsetLeft-b+"px"},m=function(){document.onmouseup=null;document.onmousemove=null};c.onmousedown=function(g){g=g||window.event;let l=g.target;!(l=l||null)||nodrag_tds.includes(l.tagName.toLowerCase())||l.classList.contains(nodrag_cls)||(g.preventDefault(),f=g.clientX,e=g.clientY,document.onmouseup=
...
setXY(b,d,f=0){this.wx=b;this.wy=d;this.pos=f;return this},setCenterY(b,d){this.setXY((window.innerWidth-this.w.clientWidth)/2,b,d);return this},setCenter(b){this.setXY((window.innerWidth-this.w.clientWidth)/2,(window.innerHeight-this.w.clientHeight)/2,b);return this},linkToId(b,d,f,e){b=document.getElementById(b);this.linkToElement(b,d,f,e);return this},linkToElement(b,d,f,e){d=b.offsetLeft+b.offsetWidth+d;b=b.offsetTop+f;0>b&&(b=0);this.setXY(d,b,e);return this},setZ(b){this.wz=b;return this},reset(){this.firstShow=
