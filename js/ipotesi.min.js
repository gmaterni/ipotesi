...
<div class="div-text">${c}</div>
    ...
    `;this.w.drag();this.w.setZ(12);this.w.vw_vh().setXY(0,10,-1);this.w.setHtml(c);this.w.show()},close(){this.w.close()},open(c){fetchText(c,b=>this.show(b))}}),wnds={wdiv:null,init(){this.wdiv=WndDiv("id_w0")},closeAll(){UaWindowAdm.close("id_w0")}},sortSchede=a=>{a.schede.sort((c,b)=>{c=parseInt(c.id,10);b=parseInt(b.id,10);return c-b});return a},MgrUi={ipotesi:[],htmlArchivio:null,htmlSommario:null,numeri:[],async init(){const a=await getJson("./data/ipotesi.json");this.setting(a.numeri)},async setting(a){this.ipotesi=
[];this.numeri=a;for(const c of this.numeri.reverse())a=await getJson(`./data/${c}/sommario.json`),a=sortSchede(a),this.ipotesi.push(a);this.buildHtml();this.showSommario()},buildHtml(){const a=(g,l)=>{g=`./data/${g}/${l.file}`;return l.img?`
        <div class="list-item">
        <a href="#" onclick="openReader('${g}')"><strong>${l.titolo}</strong></a>
        <p>${l.sottotitolo}</p>
        <p> <img src="${l.img}">   ${l.autore}  </p>
        </div>`:`
        <div class="list-item">
        <a href="#" onclick="openReader('${g}')"><strong>${l.titolo}</strong></a>
        <p>${l.sottotitolo}</p>
      ...
      <div class="num">${g.replace("n","Numero: ")}</div>
      `;const b=UaJtfh();b.init();b.append('<div class="list">');for(var d=0;d<this.ipotesi.length;d++){var f=this.ipotesi[d],k=this.numeri[d];b.append(c(k));f=f.schede;for(const g of f)b.append(a(k,g))}b.append("</div>");this.htmlArchivio=b.html();d=this.numeri[0];k=this.ipotesi[0];b.init();b.append('<div class="list">');b.append(c(d));c=k.schede;for(const g of c)b.append(a(d,g));b.append("</div>");this.htmlSommario=b.html()},showSommario(){document.getElementById("id_item1").innerHTML=MgrUi.htmlSommario},
showIndici(){document.getElementById("id_item1").innerHTML=MgrUi.htmlArchivio}};const nodrag_tds=["input","select","a"],nodrag_cls="nodrag";
var UaDrag=function(a){return function(c){let b=0,d=0,f=0,k=0;const g=function(e){e=e||window.event;e.preventDefault();b=f-e.clientX;d=k-e.clientY;f=e.clientX;k=e.clientY;c.style.top=c.offsetTop-d+"px";c.style.left=c.offsetLeft-b+"px"},l=function(){document.onmouseup=null;document.onmousemove=null};c.onmousedown=function(e){e=e||window.event;let m=e.target;!(m=m||null)||nodrag_tds.includes(m.tagName.toLowerCase())||m.classList.contains(nodrag_cls)||(e.preventDefault(),f=e.clientX,k=e.clientY,document.onmouseup=
...
setXY(b,d,f=0){this.wx=b;this.wy=d;this.pos=f;return this},setCenterY(b,d){this.setXY((window.innerWidth-this.w.clientWidth)/2,b,d);return this},setCenter(b){this.setXY((window.innerWidth-this.w.clientWidth)/2,(window.innerHeight-this.w.clientHeight)/2,b);return this},linkToId(b,d,f,k){b=document.getElementById(b);this.linkToElement(b,d,f,k);return this},linkToElement(b,d,f,k){d=b.offsetLeft+b.offsetWidth+d;b=b.offsetTop+f;0>b&&(b=0);this.setXY(d,b,k);return this},setZ(b){this.wz=b;return this},reset(){this.firstShow=
