...
function fetchJson(a,c){console.error("****fetchJson");fetch(`${a}?cacheBuster=${cacheBuster}`).then(b=>{if(!b.ok)throw Error(`HTTP error ${b.status}\nfetchJson\nurl:${a}`);return b.json()}).then(b=>c(b)).catch(b=>{console.error(`Error fetchJson ${a}`,b);alert(`Error fetchJson ${a}\n${b}`)})};function mgrVersion(){var a=document.getElementsByTagName("script");let c="0.0";for(let b of a)if(b.src&&(a=(new URL(b.src,window.location.href)).searchParams.get("v"))){c=a;break}console.log(`Versione: ${c}`);document.getElementById("id_version").innerHTML=c}function openApp(){setTimeout(()=>{initMenu();wnds.init();MgrUi.init();getTheme();imageCarousel();mgrVersion()},10)}
