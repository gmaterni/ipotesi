const cacheBuster=(new Date).getTime();async function getText(a){console.error("****getText");try{var c=await fetch(`${a}?cacheBuster=${cacheBuster}`,{headers:{"Content-Type":"text/plain"}});if(!c.ok)throw Error(`Errore HTTP: ${c.status}`);return await c.text()}catch(b){throw c=b instanceof Error?b.message:"Errore sconosciuto",console.error(`Errore getText()  ${a}:`,c),alert(`Errore getText()  ${a}\n${c}`),b;}}
...
function fetchJson(a,c){console.error("****fetchJson");fetch(`${a}?cacheBuster=${cacheBuster}`).then(b=>{if(!b.ok)throw Error(`HTTP error ${b.status}\nfetchJson\nurl:${a}`);return b.json()}).then(b=>c(b)).catch(b=>{console.error(`Error fetchJson ${a}`,b);alert(`Error fetchJson ${a}\n${b}`)})};function mgrVersion(){var a=document.getElementsByTagName("script");let c="0.0";for(let b of a)if(console.log(b),b.src&&(a=(new URL(b.src,window.location.href)).searchParams.get("v"))){c=a;break}console.log(`Versione: ${c}`);document.getElementById("id_version").innerHTML=c}function openApp(){setTimeout(()=>{initMenu();wnds.init();MgrUi.init();getTheme();imageCarousel();mgrVersion()},10)}
...
const imageCarousel=()=>{const a=document.querySelector(".col-right");a.querySelectorAll(".item0").forEach(c=>{c.classList.add("active")});setInterval(()=>{const c=a.querySelector(".item0");a.appendChild(c)},5E3)};let fontSize=18;const fsizeDef=18,fsizeMax=32,fsizeMin=14,fsizeKey="fsize_reader";var textCurrent="";const fh=a=>`
...
`,showReader=a=>{textCurrent=a;h=fh(a);a=UaWindowAdm.create("id_reader");a.setZ(12);a.vw_vh().setXY(.1,10,-1);a.setHtml(h);a.show();defaultFontSize()};function openReader(a){fetchText(a,showReader);enableEsc()}
...
[];this.numeri=a;for(const c of this.numeri)a=await getJson(`./data/${c}/sommario.json`),a=sortSchede(a),this.ipotesi.push(a);this.buildHtml();this.showSommario()},buildHtml(){const a=(f,k)=>{f=`./data/${f}/${k.file}`;return k.img?`
      ...
      `);d=d.schede;for(const f of d)c.append(a(e,f))}c.append("</div>");this.htmlIndici=c.html();e=this.ipotesi.length-1;b=this.ipotesi[e];e=this.numeri[e];c.init();c.append('<div class="list">');b=b.schede;for(const f of b)c.append(a(e,f));c.append("</div>");this.htmlSommario=c.html()},showSommario(){document.getElementById("id_item1").innerHTML=MgrUi.htmlSommario},showIndici(){document.getElementById("id_item1").innerHTML=MgrUi.htmlIndici}};const nodrag_tds=["input","select","a"],nodrag_cls="nodrag";
